...
func _process(delta: float) -> void:
	...
	if is_instance_valid(target_node):
		var target_center: Vector2 = target_node.call("get_hit_center") if target_node.has_method("get_hit_center") else target_node.position
		var desired_direction = (target_center - position).normalized()
...
func _update_target_reference() -> void:
	...
	for target in get_tree().get_nodes_in_group("targets"):
		...
		var d = position.distance_to(target_center)
		if d < best_distance:
			best_distance = d
			best_target = target
...
func _check_target_collision() -> void:
	for target in get_tree().get_nodes_in_group("targets"):
		if not is_instance_valid(target) or target.is_dead:
			continue
		var target_center: Vector2 = target.call("get_hit_center") if target.has_method("get_hit_center") else target.position
		if position.distance_to(target_center) <= target.get_hit_radius():
			target.take_hit()
			_finish(true)
			return
...
func _draw() -> void:
	...
	if is_instance_valid(target_node):
		var target_center: Vector2 = target_node.call("get_hit_center") if target_node.has_method("get_hit_center") else target_node.position
		var local_target = target_center - position
